...
export default function InventoryCard({ product }: { product: IProduct }) {
    ...
    return (
        <div className={`flex flex-col bg-white dark:bg-[#000]/[0.1] border border-gray-500/[0.2] relative break-inside-avoid md:mb-4 mb-2 pb-4 `} data-aos="fade-up">
                <Link href={`/dashboard/inventory/edit?id=${product?._id}`} className="block rounded sm:h-[250px] h-[250px] bg-gray-500/[0.1] bg-cover bg-center" style={{backgroundImage: `url("${product?.images[0]}")`}} >
                </Link>
                
                <p className="text-[10px] opacity-[0.5] uppercase font-bold px-3 py-0 my-2">{product?.category}</p>
                <a  href={`/dashboard/inventory/edit?id=${product?._id}`} className="block pb-4 px-3 leading-[130%] text-[12px] font-semibold">{product?.title}</a>
